...
passport.use('local', new LocalStrategy({
    ...
    try {
        ...
        var currentAccountsData = await JSON.stringify(client.query('SELECT * FROM cuentas WHERE usuario=$1', [username], function (err, result) {
            if (err) {
                console.log(err);
                return done(err)
            }
            if (result.rows[0] == null) {
                ...
                console.log('1:', req.body.usuario, req.body.password, password);
